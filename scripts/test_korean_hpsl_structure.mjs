...
import { analyzeScriptStructure, splitKoreanSentences } from "./lib/script-structure-analysis.mjs";
...
const sentences = splitKoreanSentences(chapterOneParagraphs.join(" "));
assert(sentences.length >= 8, `expected Korean sentence splitter to find sentences, got ${sentences.length}`);

const structure = analyzeScriptStructure(script, { minChapters: 2 });
assert.equal(structure.ok, true, JSON.stringify(structure, null, 2));

const hpsl = analyzeScriptHpsl(script, { minChapterPassRate: 1 });
assert.equal(hpsl.ok, true, JSON.stringify(hpsl, null, 2));

console.log("test_korean_hpsl_structure: pass");
